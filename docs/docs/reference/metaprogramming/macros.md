...
This two aspects are also important for macro expansion.
For instance, a constant expression passed as argument to an inline parameter
can potentially result in a constant expression as well and thus further 
be folded after inlining the `inline def`.

`Expr` provides the method `getValue` (and `value`).
This will convert the `Expr[T]` into a `Some[T]` (or `T`) when the expression 
corresponds to a single constant value (after constant folding). 
Otherwise it will return `None` (or emit an error).
...
```
<!-- does the compiler not detect incidental val bindings?
If not, would it result in a weird error (potentially at runtime)? 
IMO this should be documented and it would be great to see a problematic situation -->
